...
hid_num = size(Theta1, 1);
for i = 1:m
	% Feedforward and cost function
	a2 = [];
	z2 = [];
	for j=1:hid_num
		z = X(i,:) * transpose(Theta1(j,:));
		z2 = [z2; z];
		a2 = [a2; (sigmoid(z))];
	end
	...
	yy(y(i,1), 1) = 1;
	% Add one to a2 matrix
	a2 = [1; a2];
	a3 = [];
	delta3 = [];
	
	for k=1:num_labels
		% Feedforward and cost function for a3
		a3 = [a3; (sigmoid(Theta2(k,:) * a2))];
		J += (-yy(k,1) * log(a3(k,1)) - (1-yy(k,1)) * log(1-a3(k,1)));
		% Backpropagation for delta3
		delta3 = [delta3; (a3(k,1) - yy(k,1))];
	end
